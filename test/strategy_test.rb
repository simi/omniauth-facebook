...
class ClientTest < StrategyTestCase
  test 'has correct Facebook site' do
    assert_equal 'https://graph.facebook.com/v5.0', strategy.client.site
  end

  test 'has correct authorize url' do
    assert_equal 'https://www.facebook.com/v5.0/dialog/oauth', strategy.client.options[:authorize_url]
  end
...
class InfoTest < StrategyTestCase
  ...
  test 'returns the secure facebook avatar url when `secure_image_url` option is set to true' do
    ...
    assert_equal 'https://graph.facebook.com/v5.0/321/picture?access_token=test_access_token', strategy.info['image']
  end
  ...
  test 'returns the non-ssl facebook avatar url when `secure_image_url` option is set to false' do
    ...
    strategy.stubs(:raw_info).returns(raw_info)
    strategy.stubs(:access_token).returns(@access_token)
    assert_equal 'http://graph.facebook.com/v5.0/321/picture?access_token=test_access_token', strategy.info['image']
  end
  ...
  test 'returns the image with size specified in the `image_size` option' do
    ...
    strategy.stubs(:raw_info).returns(raw_info)
    strategy.stubs(:access_token).returns(@access_token)
    assert_equal 'https://graph.facebook.com/v5.0/321/picture?access_token=test_access_token&type=normal', strategy.info['image']
  end
  ...
  test 'returns the image with width and height specified in the `image_size` option' do
    ...
    assert_match 'https://graph.facebook.com/v5.0/321/picture?access_token=test_access_token', strategy.info['image']
  end
end
...
class RawInfoTest < StrategyTestCase
  ...
  test 'performs a GET to https://graph.facebook.com/v5.0/me' do
  ...
  test 'performs a GET to https://graph.facebook.com/v5.0/me with locale' do
  ...
  test 'performs a GET to https://graph.facebook.com/v5.0/me with info_fields' do
  ...
  test 'performs a GET to https://graph.facebook.com/v5.0/me with default info_fields' do
    strategy.stubs(:access_token).returns(@access_token)
    strategy.stubs(:appsecret_proof).returns(@appsecret_proof)
