...
class ClientTest < StrategyTestCase
  test 'has correct Facebook site' do
    assert_equal 'https://graph.facebook.com/v2.7', strategy.client.site
  end

  test 'has correct authorize url' do
    assert_equal 'https://www.facebook.com/v2.7/dialog/oauth', strategy.client.options[:authorize_url]
  end
...
class InfoTest < StrategyTestCase
  test 'returns the secure facebook avatar url when `secure_image_url` option is specified' do
    ...
    assert_equal 'https://graph.facebook.com/v2.7/321/picture', strategy.info['image']
  end
  ...
  test 'returns the image with size specified in the `image_size` option' do
    ...
    raw_info = { 'name' => 'Fred Smith', 'id' => '321' }
    strategy.stubs(:raw_info).returns(raw_info)
    assert_equal 'http://graph.facebook.com/v2.7/321/picture?type=normal', strategy.info['image']
  end
  ...
  test 'returns the image with width and height specified in the `image_size` option' do
    ...
    assert_match 'http://graph.facebook.com/v2.7/321/picture?', strategy.info['image']
  end
end
...
class InfoTestOptionalDataPresent < StrategyTestCase
  ...
  test 'returns the facebook avatar url' do
    @raw_info['id'] = '321'
    assert_equal 'http://graph.facebook.com/v2.7/321/picture', strategy.info['image']
  end
...
class RawInfoTest < StrategyTestCase
  ...
  test 'performs a GET to https://graph.facebook.com/v2.7/me' do
  ...
  test 'performs a GET to https://graph.facebook.com/v2.7/me with locale' do
  ...
  test 'performs a GET to https://graph.facebook.com/v2.7/me with info_fields' do
  ...
  test 'performs a GET to https://graph.facebook.com/v2.7/me with default info_fields' do
    strategy.stubs(:access_token).returns(@access_token)
    strategy.stubs(:appsecret_proof).returns(@appsecret_proof)
